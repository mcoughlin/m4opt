...
def _bisect_graph_milp(graph, **kwargs):
    ...
    if len(components) >= 2:
        ...
        # Bisection failed; fall back to heuristic
...
def partition_graph_milp_recursive(
    ...
    **kwargs,
    ...
    For small subgraphs (<= 500 nodes), each bisection uses the MILP flow
    formulation from Section 4 of :footcite:`2019arXiv191105723M` with SOS1
    constraints and k=2. For larger subgraphs, a fast Kernighan-Lin heuristic
    is used with connectivity repair.

    This reduces the problem from a single k-partition MILP (intractable for
    large k) to a sequence of bisection problems of decreasing size.
